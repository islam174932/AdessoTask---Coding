...
Then("I should see pet {string} with birth date {string} and type {string}", async function (pet, birth, type) {
  const petRow = await this.page!.locator(`.ownerPets tr:has-text('${pet}')`).first();
