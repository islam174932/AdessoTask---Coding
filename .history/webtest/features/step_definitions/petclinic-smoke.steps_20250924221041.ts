...
Then("I should see the Find Owners icon", async function (this: CustomWorld) {
  ...
  const findOwnersSelector =
    'a.nav-link[href="/owners/find"] span:has-text("Find Owners")';
...
Then("I should see the Veterinarians icon", async function (this: CustomWorld) {
  // Use Playwright's getByRole for robust selection
  const vetLink = this.page!.getByRole("link", { name: /Veterinarians/ });
  expect(await vetLink.isVisible()).toBeTruthy();
});
