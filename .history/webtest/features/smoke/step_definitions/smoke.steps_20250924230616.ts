...
Then("I should see the Find Owners header", async function () {
  expect(await this.page!.getByRole("heading", { name: /Find Owners/ }).isVisible()).toBeTruthy();
});
...
Then("I should see the Veterinarians header", async function () {
  expect(await this.page!.getByRole("heading", { name: /Veterinarians/ }).isVisible()).toBeTruthy();
});
