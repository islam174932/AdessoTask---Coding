...
const { PetPage } = require("../../src/pages/PetPage.js");
const { VisitPage } = require("../../src/pages/VisitPage.js");
...
When("I click the owner link {string} with address {string}", async function (fullName, address) {
  const row = this.ownersPage.getOwnerRowByName(fullName).filter({ hasText: address }).first();
