...
When(
  ...
  async function (this: CustomWorld) {
    ...
    // Try both possible button names for Add Owner form
    try {
...
Then(
  ...
  async function (
    ...
    const isOwnerPage =
      (await this.page.getByRole("heading", { name: "Owner Information" }).count()) > 0;
    if (isOwnerPage) {
      ...
      const ownerRow = this.page.locator(`table tbody tr:has-text("${ownerName}")`);
...
When(
  ...
  async function (this: CustomWorld) {
    ...
    const initialOwnerName = `${initialOwner.firstName || ""} ${initialOwner.lastName || ""}`.trim();
    const initialAddress = initialOwner.address || "";

    let ownerLink = this.page.locator(`a:has-text("${initialOwnerName}")`);
    if ((await ownerLink.count()) === 0 && initialAddress) {
    ...
    if ((await ownerLink.count()) === 0) {
      ...
      const updatedOwnerName = `${updatedOwner.firstName || ""} ${updatedOwner.lastName || ""}`.trim();
      const updatedAddress = updatedOwner.address || "";
      ownerLink = this.page.locator(`a:has-text("${updatedOwnerName}")`);
      if ((await ownerLink.count()) === 0 && updatedAddress) {
    ...
    if ((await ownerLink.count()) === 0) {
      ...
      throw new Error(`Owner link not found for either ${initialOwnerName} or updated owner.`);
    }
