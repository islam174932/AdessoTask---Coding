...
When('I enter visit details:', async function (dataTable) {
  ...
  // Use type to fill the description field and trigger validation
  if (data.description) {
    ...
    await descInput.type(data.description, { delay: 50 }); // Type the description value
    await page.waitForTimeout(200); // Wait after typing
  }
});
