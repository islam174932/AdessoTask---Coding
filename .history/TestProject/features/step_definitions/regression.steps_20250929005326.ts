...
Then(
  ...
  const filteredRow = ownerRows.filter({ hasText: address }).filter({ hasText: city }).filter({ hasText: telephone }).first();
...
When("I click the Edit Owner button", async function (this: CustomWorld) {
  ...
  await this.page.waitForSelector('h2:has-text("Owner Information")', { timeout: 20000 });
  await this.page.waitForSelector('a:has-text("Edit Owner")', { timeout: 20000 });
  await this.page.click('a:has-text("Edit Owner")');
  await this.page.waitForSelector('h2:has-text("Edit Owner")', { timeout: 20000 });
});
...
Then(
  ...
  async function (this: CustomWorld) {
    ...
    await this.page.waitForSelector('h2:has-text("Owner Information")', { timeout: 10000 });
...
Then(
  ...
  async function (this: CustomWorld, message: string) {
    ...
    await this.page.waitForSelector('.alert, .success, .message', { timeout: 20000 });
    await expect(this.page.locator('.alert, .success, .message')).toContainText(message);
  }
);
...
When("I click the pet name link from JSON", async function (this: CustomWorld) {
  ...
  await this.page.waitForSelector(`a:has-text("${petName}")`, { timeout: 10000 });
...
When("I submit the Add Visit form", async function (this: CustomWorld) {
  const visitPage = new (require("../../src/pages/VisitPage").VisitPage)(this.page);
