...
When('I enter visit details:', async function (dataTable) {
  ...
  if (data.date) {
    const dateInput = await page.locator('input[name="date"]');
    await dateInput.fill(''); // Clear field first
    await dateInput.type(data.date); // Type the date value
  }
  if (data.description) {
...
Then('I should see the new visit with date {string} and description {string} for pet {string}', async function (date, description, petName) {
  const page = this.page;
  // Check Previous Visits table for the new visit
  await page.waitForSelector('table:has-text("Previous Visits")');
  const visitRow = await page.locator(`table:has-text('Previous Visits') tr:has-text('${date}')`).first();
  const rowText = await visitRow.textContent();
  if (!rowText || !rowText.includes(description)) {
    throw new Error(`Visit with date ${date} and description ${description} not found for pet ${petName}`);
  }
});
