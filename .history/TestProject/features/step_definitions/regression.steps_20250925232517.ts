...
When(
  "I enter owner details from JSON",
  { timeout: 60000 },
  async function (this: CustomWorld) {
    const owner = testData.owner;
    await this.addOwnerPage!.firstNameInput().waitFor({ state: "visible", timeout: 30000 });
...
Then("I should see the new owner added", async function (this: CustomWorld) {
  const owner = this.lastOwner || testData.owner;
  const fullName = `${owner.firstName} ${owner.lastName}`;
  await this.page!.waitForTimeout(2000);
  await expect(this.page!.getByText(fullName)).toBeVisible({ timeout: 10000 });
  await expect(this.page!.getByText(owner.address)).toBeVisible({ timeout: 10000 });
  await expect(this.page!.getByText(owner.city)).toBeVisible({ timeout: 10000 });
  await expect(this.page!.getByText(owner.telephone)).toBeVisible({ timeout: 10000 });
});

Then("I should see the confirmation message {string}", { timeout: 20000 }, async function (this: CustomWorld, message: string) {
  const successDiv = this.page!.locator('div.alert-success#success-message');
  await successDiv.waitFor({ state: 'attached', timeout: 15000 });
  await expect(successDiv.locator('span', { hasText: message })).toHaveCount(1, { timeout: 5000 });
});
...
When(
  "I update owner details from JSON",
  { timeout: 60000 },
  async function (this: CustomWorld) {
    console.log("Starting to update owner details...");
    const owner = testData.updatedOwner;
    console.log("Updated owner data:", owner);
...
Then(
  ...
  async function (this: CustomWorld, petName, birthDate, type, ownerName) {
    ...
    await expect(
      this.page!.getByText(expectedPetName, { exact: true })
    ).toBeVisible({
      timeout: 10000,
    });
...
Then("I should see the new pet from JSON", async function (this: CustomWorld) {
  ...
  const birthDateLocator = this.page!.locator("dd", {
    hasText: testData.pet.birthDate,
  });
