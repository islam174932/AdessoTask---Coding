...
When('I enter visit details:', async function (dataTable) {
  ...
  if (data.date) {
    const dateInput = await page.locator('input[type="date"][name="date"]');
    await dateInput.fill(''); // Clear field first
    await dateInput.type(data.date); // Type the date value
  }
  if (data.description) {
    await page.fill('input[type="text"][name="description"]', data.description);
  }
});
...
Then('I should see the new visit with date {string} and description {string} for pet {string}', async function (date, description, petName) {
  ...
  // Find the row in Previous Visits table
  const visitRow = await page.locator(`table:has-text('Previous Visits') tr`).filter({ hasText: date }).first();
