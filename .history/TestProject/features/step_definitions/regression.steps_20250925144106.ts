...
When('I enter visit details:', async function (dataTable) {
  ...
  if (data.description) {
    // Wait 40 seconds before any input
    await page.waitForTimeout(40000);
    try {
      const descBox = page.getByRole('textbox', { name: 'Description' });
      await descBox.click();
      await descBox.fill(data.description);
      // Validate the value is set
      const value = await descBox.inputValue();
      if (value !== data.description) {
        throw new Error(`Description field not filled correctly. Expected: ${data.description}, Actual: ${value}`);
      }
    } catch {
      // Fallback: try textarea selector
      const fallback = page.locator('textarea');
      await fallback.click();
      await fallback.fill(data.description);
      const value = await fallback.inputValue();
      if (value !== data.description) {
        throw new Error(`Description field not filled correctly (fallback). Expected: ${data.description}, Actual: ${value}`);
      }
    }

    // Take screenshot after submitting the form for debugging
    await page.screenshot({ path: `screenshots/add-visit-after-submit.png`, fullPage: true });
  }
});
