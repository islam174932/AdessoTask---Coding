...
When('I click the owner link {string}', async function (this: CustomWorld, fullName: string) {
  // Use address for uniqueness
  const owner = this.lastOwner || {};
  ...
  for (let i = 0; i < count; i++) {
    ...
    if (await nameLink.isVisible() && await addressCell.textContent() === address) {
