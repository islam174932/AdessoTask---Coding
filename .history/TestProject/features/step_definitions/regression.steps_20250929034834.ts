...
When("I click the pet name link {string}", async function (petName) {
  // If we have a current pet name from the add pet step, use that
  // Otherwise use the parameter from the feature file
  const actualPetName = this.currentPetName || petName;
  const petLink = this.petPage.petNameLink(actualPetName);
