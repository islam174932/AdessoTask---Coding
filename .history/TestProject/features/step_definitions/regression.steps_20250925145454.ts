...
When("I enter visit details:", async function (dataTable) {
  ...
  if (data.description) {
    ...
    try {
      ...
      console.log("Standard fill failed, trying force fill...");
      // Approach 2: Force fill
      await descBox.fill(data.description, { force: true });
    }

    // Verify it was filled
    await page.waitForTimeout(500); // Wait for value to be processed
    const filledValue = await descBox.inputValue();
    console.log("Description filled with:", filledValue);

    // If still empty, try keyboard typing
    if (!filledValue || filledValue !== data.description) {
      console.log("Fill failed, trying keyboard typing...");
      await descBox.click();
      await page.keyboard.press("Control+A");
      await page.keyboard.type(data.description);

      // Verify again
      const finalValue = await descBox.inputValue();
      console.log("Final value after typing:", finalValue);
    }
  }
});

// Keep your separate submit step:
