...
When('I enter visit details:', async function (dataTable) {
  ...
  if (data.description) {
    // Debugging: check if element exists, is visible, and is enabled
    const element = await page.$('#description');
    console.log('Element exists:', !!element);
    const isVisible = await page.isVisible('#description');
    console.log('Element visible:', isVisible);
    const isEnabled = await page.isEnabled('#description');
    ...
    if (value !== data.description) {
      throw new Error(`Description field value is '${value}', expected '${data.description}'`);
    }
  }
});
