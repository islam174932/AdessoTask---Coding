...
try {
  ...
  testData = testData || {};
  testData.initialOwner = testData.initialOwner || {
    firstName: "Islam",
    lastName: "Ibrahim",
    address: "000 Pine Rd.",
    city: "Cairo",
  ...
  testData.updatedOwner = testData.updatedOwner || {
    firstName: "Layla",
    lastName: "Hassan",
    address: "123 Oak St.",
    city: "Damascus",
  ...
  testData.newPet = testData.newPet || {
    name: "Fluffy",
    birthDate: "2023-01-15",
    type: "cat",
  };
  testData.newVisit = testData.newVisit || {
    date: "2024-01-20",
...
When(
  ...
  async function (this: CustomWorld) {
    ...
    await this.page.waitForSelector('h2:has-text("New Owner")', { timeout: 10000 });
...
Then(
  ...
  async function (
    ...
    const isOwnerPage = (await this.page.locator('h2').filter({ hasText: "Owner Information" }).count()) > 0;
    if (isOwnerPage) {
