...
When("I enter owner details from JSON", async function (this: CustomWorld) {
  ...
  // Wait for the Add Owner form to be visible
  await this.page!.waitForTimeout(5000); // Extra wait before checking form
  await this.page!.waitForSelector('form#add-owner-form, form[action*="owners/new"]', { state: "visible", timeout: 60000 });
  await this.addOwnerPage!.firstNameInput().waitFor({ state: "visible", timeout: 60000 });
...
Then("I should see the updated owner from JSON", async function (this: CustomWorld) {
  const owner = testData.updatedOwner;
  const fullName = `${owner.firstName} ${owner.lastName}`;
