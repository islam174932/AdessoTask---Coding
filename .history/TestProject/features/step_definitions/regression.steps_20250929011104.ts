...
When(
  ...
  async function (this: CustomWorld) {
    // Find the correct owner link by href and name
    const owner = testData.updatedOwner || testData.initialOwner;
    const ownerName = `${owner.firstName} ${owner.lastName}`;
    const ownerLink = this.page.locator(`a[href^="/owners/"]:has-text("${ownerName}")`).first();
...
When("I click the pet name link from JSON", async function (this: CustomWorld) {
  ...
  await this.page.waitForSelector(`a:has-text("${petName}")`, {
    timeout: 10000,
  });
