...
Given('I open the home page', async function (this: CustomWorld) {
  ...
  this.homePage = new (await import('../../src/pages/HomePage.ts')).HomePage(this.page!);
});
...
When('I enter visit details:', async function (dataTable) {
  ...
  if (data.description) {
    ...
    // Log selector existence
    const descVisible = await page.isVisible('#description');
    console.log(`[Test] #description visible: ${descVisible}`);
    // Click the description field to focus it
    await page.click('#description');
    // Log click
    console.log('[Test] Clicked #description');
    // Clear any existing value
    await page.fill('#description', '');
    // Log fill
    console.log('[Test] Cleared #description');
    // Type the description slowly to trigger any JS events
    await page.type('#description', data.description, { delay: 100 });
    console.log(`[Test] Typed description: ${data.description}`);
    // Validate the value is set
    const value = await page.inputValue('#description');
    console.log(`[Test] #description value after typing: ${value}`);
    if (value !== data.description) {
      throw new Error(`Description field not filled correctly. Expected: ${data.description}, Actual: ${value}`);
    }
  }
});
...
Then('I should see the new visit with date {string} and description {string} for pet {string}', async function (this: CustomWorld, date: string, description: string, petName: string) {
  const page = this.page!;
  // Wait for Previous Visits table to appear and update
  await page.waitForSelector('b:has-text("Previous Visits") + table.table-striped', { timeout: 15000 });
