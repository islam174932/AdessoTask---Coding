...
When("I enter visit details from JSON", async function (this: CustomWorld) {
  ...
  const visitPage = new (require("../../src/pages/VisitPage")?.VisitPage ?? class { constructor(page) { this.page = page; } dateInput() { return this.page.locator('#date'); } descriptionInput() { return this.page.locator('#description'); } })(this.page);
...
When("I submit the Add Visit form", async function (this: CustomWorld) {
  await this.page.click('button[type="submit"]'); // If VisitPage has a submit method, use it here
  await this.page.waitForLoadState("networkidle");
});
