...
When("I enter owner details from json for index {int}", async function (index) {
  try {
    ...
    await this.page.screenshot({ path: "after-form-fill.png", fullPage: true });
  } catch (error) {
    console.log("ERROR during form filling:", error.message);
    await this.page.screenshot({ path: "form-fill-error.png", fullPage: true });
    throw error;
  }
});
...
Then("I should see the new owner added", async function () {
  ...
  // Check for owner details page instead of owners table
  await this.page.waitForSelector('h2:has-text("Owner Information")', {
    timeout: 10000,
  });

  // Validate owner details in the details table
