...
When("I enter visit details:", async function (dataTable) {
  ...
  if (data.description) {
    // Wait for form to be ready (reduce from 40 seconds to reasonable time)
    await page.waitForLoadState("networkidle");
    await page.waitForTimeout(2000);

    // DON'T click "Add Visit" again - you should already be on the form page
    // Remove this line: await page.getByRole("link", { name: "Add Visit" }).first().click();

    // Fill the date field first (if provided)
    if (data.date) {
      await page.getByRole("textbox", { name: "Date" }).fill(data.date);
    }

    // Fill description field with multiple fallbacks
    try {
      const descBox = page.getByRole("textbox", { name: "Description" });
      await descBox.waitFor({ state: "visible" });
      await descBox.click();
      await descBox.fill(data.description);
    } catch (error1) {
      try {
        // Fallback 1: Direct textarea selector
        await page.locator("textarea").fill(data.description);
      } catch (error2) {
        // Fallback 2: By name attribute
        await page
    ...
    const filledValue = await page
      .getByRole("textbox", { name: "Description" })
      .inputValue();
    console.log("Description field value:", filledValue);
    ...
    // DON'T submit here - do it in a separate step
    // Remove this line: await page.getByRole("button", { name: "Add Visit" }).click();
  }
});

// Separate step for submission
