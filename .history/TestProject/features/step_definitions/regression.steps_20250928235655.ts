...
try {
  ...
  console.log("Test data file not found, using default data");
  // Default test data based on your flow
  testData = {
    // Initial owner to be created
    initialOwner: {
      firstName: "Islam",
      lastName: "Ibrahim",
      address: "000 Pine Rd.",
    ...
    // Updated owner (Islam becomes Layla)
    updatedOwner: {
      firstName: "Layla",
      lastName: "Hassan",
      address: "123 Oak St.",
...
// Store created owner ID for later use
...
Given(
  ...
  async function (this: CustomWorld) {
    // Use HomePage (inherits BasePage) for opening the home page
...
// Step 1: Create the initial owner (Islam Ibrahim)
When(
  ...
  async function (this: CustomWorld) {
    ...
    // Store owner ID from URL or page for later use
    const currentUrl = this.page.url();
    const match = currentUrl.match(/owners\/(\d+)/);
    if (match) {
      createdOwnerId = match[1];
      console.log(`Created owner with ID: ${createdOwnerId}`);
    }
  }
);
