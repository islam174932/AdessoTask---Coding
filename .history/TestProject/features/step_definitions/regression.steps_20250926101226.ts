...
Then(
  ...
  async function (this: CustomWorld, message: string) {
    ...
    await expect(successDiv.locator("span", { hasText: message })).toHaveCount(
      1,
      { timeout: 5000 }
    );
  }
...
When(
  ...
  async function (this: CustomWorld) {
    // Use updatedOwner for scenarios that run after the update
    await this.ownersPage!.lastNameInput().fill(testData.updatedOwner.lastName);
  }
);
...
When(
  ...
  async function (this: CustomWorld) {
    // Use updatedOwner data for scenarios that run after the update
    const fullName = `${testData.updatedOwner.firstName} ${testData.updatedOwner.lastName}`;
...
When("I click the pet name link from JSON", async function (this: CustomWorld) {
  ...
  // Use the dynamically generated pet name if available
