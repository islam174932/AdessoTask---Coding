When("I click the first owner link for {string} in the owners table", async function (this: CustomWorld, ownerName: string) {
...
Then("the owner link for {string} with id {string} should be visible in the owners table", async function (this: CustomWorld, ownerName: string, ownerId: string) {
  const ownerLink = ownersPage.getOwnerLinkByNameAndId(ownerName, ownerId);
  await ownerLink.waitFor({ state: "visible", timeout: 10000 });
...
When(
  ...
  async function (this: CustomWorld) {
    ...
    // Always click the first owner link for the initial owner's name after search
    const owner = testData.initialOwner;
