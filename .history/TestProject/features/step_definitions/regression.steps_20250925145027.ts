...
When("I enter visit details:", async function (dataTable) {
  ...
  if (data.description) {
    // Reduce wait time - 40 seconds is excessive
    await page.waitForLoadState("networkidle");
    await page.waitForTimeout(2000);

    // REMOVE THIS LINE - you should already be on the form page
    ...
    // REMOVE THIS LINE - submit should be in separate step
    // await page.getByRole('button', { name: 'Add Visit' }).click();

    // Take screenshot after filling (before submit)
...
And("I submit the Add Visit form", async function () {
  const page = this.page!;
