...
When(
  ...
  async function (this: CustomWorld) {
    ...
    // Use robust selector for Add Owner button
    try {
      await this.page.getByRole("link", { name: "Add Owner" }).click();
    } catch {
      await this.page.click('a.btn.btn-primary[href="/owners/new"]');
    }
    await this.page.getByRole("heading", { name: "New Owner" }).waitFor({ timeout: 20000 });
    ...
    // Submit form (try both possible button names)
    try {
