...
Given("I open the home page", async function (this: CustomWorld) {
  ...
  try {
    ...
    console.log("HomePage module not found, will use direct selectors for home functionality");
    this.homePage = null;
  }
});
...
When("I click the Home icon", async function (this: CustomWorld) {
  if (this.homePage && this.homePage.clickHome) {
    ...
    try {
      ...
      await this.page!.locator('.navbar-brand, a[href="/"], a[href*="home"]').first().click();
    }
  }
});
...
Then("I should see the Welcome header", async function (this: CustomWorld) {
  if (this.homePage && this.homePage.isWelcomeVisible) {
    ...
    for (const text of welcomeTexts) {
      try {
        await expect(this.page!.getByText(text, { exact: false })).toBeVisible({ timeout: 3000 });
        found = true;
    ...
    if (!found) {
      await expect(this.page!.locator("h1, h2, main, .container").first()).toBeVisible({ timeout: 10000 });
    }
  }
});
...
Then(
  ...
  async function (this: CustomWorld) {
    if (this.ownersPage!.validateDavisOwners) {
      ...
      const davisEntries = this.page!.locator("table tbody tr").filter({ hasText: "Davis" });
...
Then(
  ...
  async function (this: CustomWorld) {
    ...
    for (const header of specialtyHeaders) {
      try {
        await expect(this.page!.getByText(header)).toBeVisible({ timeout: 2000 });
        break;
      } catch (e) {}
    }
...
Then(
  ...
  async function (this: CustomWorld) {
    ...
    for (const headerText of errorHeaders) {
      try {
        await expect(this.page!.getByText(headerText, { exact: false })).toBeVisible({ timeout: 3000 });
        headerFound = true;
        break;
      } catch (e) {}
    }
