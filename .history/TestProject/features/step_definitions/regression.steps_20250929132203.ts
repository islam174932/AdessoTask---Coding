...
When(
  ...
  async function (this: CustomWorld, scenarioName: string) {
    ...
    // Fix: Use fromOwnerIndex for updateOwner scenario
    const ownerIndex =
      scenario.fromOwnerIndex !== undefined
        ? scenario.fromOwnerIndex
        : scenario.ownerIndex;
    const owner = testData.owners[ownerIndex];
...
When("I enter pet details from test scenario {string}", async function (this: CustomWorld, scenarioName: string) {
  const testData = await getTestData();
  const scenario = testData.testScenarios[scenarioName];
  const pet = testData.pets[scenario.petIndex];
  
  if (!pet) throw new Error(`No pet data found for scenario: ${scenarioName}`);

  // Generate unique name using random words instead of timestamp
  const adjectives = ['Fluffy', 'Happy', 'Lucky', 'Shadow', 'Sunny', 'Misty', 'Sparkle', 'Golden', 'Silver', 'Rusty'];
  const suffixes = ['Paws', 'Tail', 'Whiskers', 'Ears', 'Nose', 'Fur', 'Star', 'Moon', 'Cloud', 'Dream'];
  
  const randomAdjective = adjectives[Math.floor(Math.random() * adjectives.length)];
  const randomSuffix = suffixes[Math.floor(Math.random() * suffixes.length)];
  const uniqueName = `${pet.name}_${randomAdjective}${randomSuffix}`;
