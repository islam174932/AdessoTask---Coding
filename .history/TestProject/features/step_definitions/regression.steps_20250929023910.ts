...
When("I enter owner details from json for index {int}", async function (index) {
  ...
  await this.addOwnerPage.lastNameInput().waitFor({ state: "visible", timeout: 10000 });
  await this.addOwnerPage.addressInput().waitFor({ state: "visible", timeout: 10000 });
  await this.addOwnerPage.cityInput().waitFor({ state: "visible", timeout: 10000 });
  await this.addOwnerPage.telephoneInput().waitFor({ state: "visible", timeout: 10000 });
