...
When('I enter visit details:', async function (dataTable) {
    const page = this.page!;
    const data = dataTable.hashes()[0]; // Get the data from the Cucumber table

    // Since the date is pre-populated, we only need to fill the description.
...
When('I submit the Add Visit form', async function () {
    ...
    // Wait for the URL to change to the expected pets details page
    await page.waitForURL('**/owners/**/pets/**');

    // Optionally, wait for a specific element on the new page to appear
    // await page.waitForSelector('h2:has-text("Pet Details")');
});
