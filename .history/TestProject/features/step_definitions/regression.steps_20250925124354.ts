...
When('I enter visit details:', async function (dataTable) {
  ...
  // Force fill the description field with extra steps
  const descInput = page.locator('input[type="text"][name="description"]');
  await descInput.click(); // Focus the field
  await page.waitForTimeout(500); // Wait for field to be ready
  await descInput.fill(''); // Clear field
  await page.waitForTimeout(500); // Wait for clear
  if (data.description) {
    await descInput.type(data.description, { delay: 100 }); // Type the description value slowly
    await page.waitForTimeout(500); // Wait after typing
  }
});
