...
When("I enter pet details from json for index {int}", async function (index) {
  ...
  this.currentPetName = uniqueName; // Store for validation
...
Then(
  ...
  async function (petName, birthDate, type, ownerName) {
    ...
    await expect(petsTable).toContainText(this.currentPetName); // Use generated name
...
When("I click the pet name link for current pet", async function () {
  try {
    // Look for any pet link on the page (since we don't have currentPetName in this scenario)
...
When("I click the Add Visit button", async function () {
  try {
    // Wait for owner details page to load completely
    ...
    // Check what visit-related elements exist
    const visitLinks = await this.page.locator('a:has-text("Visit")').all();
    console.log(`Found ${visitLinks.length} links containing "Visit"`);

    const allLinks = await this.page.locator("a").all();
    console.log(`Total links on page: ${allLinks.length}`);

    // Try the click with explicit wait
    const addVisitButton = this.visitPage.addVisitButton();
    await addVisitButton.waitFor({ state: "visible", timeout: 10000 });
    await addVisitButton.click();
  } catch (error) {
    ...
    console.log(
      'Page HTML contains "Add Visit":',
      pageContent.includes("Add Visit")
    );
...
When("I enter visit details from json for index {int}", async function (index) {
  try {
    ...
    this.currentVisit = visit; // Store for validation

    // Fill visit description
