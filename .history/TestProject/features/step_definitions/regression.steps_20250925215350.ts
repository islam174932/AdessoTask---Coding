...
Then("I should see the new pet from JSON", async function (this: CustomWorld) {
  ...
  await expect(this.page!.getByText(petName, { exact: true })).toBeVisible({ timeout: 10000 });
  await expect(this.page!.getByText(testData.pet.birthDate)).toBeVisible({ timeout: 10000 });
  await expect(this.page!.getByText(testData.pet.type)).toBeVisible({ timeout: 10000 });
});
