...
When('I update owner details:', async function (this: CustomWorld, dataTable) {
  const owner = dataTable.hashes()[0];
  await this.page.locator('input[name="firstName"]').fill('');
  await this.page.locator('input[name="lastName"]').fill('');
  await this.page.locator('input[name="address"]').fill('');
  await this.page.locator('input[name="city"]').fill('');
  await this.page.locator('input[name="telephone"]').fill('');
