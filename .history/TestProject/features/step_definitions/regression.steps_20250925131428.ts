...
const regressionTag = { tags: '@regression' };

Given('I open the home page', regressionTag, async function (this: CustomWorld) {
  await this.page.goto('http://localhost:8081/');
  this.ownersPage = new OwnersPage(this.page);
});

When('I click the Find Owners link', regressionTag, async function (this: CustomWorld) {
  await this.ownersPage.findOwnersNav().click();
});

When('I click the Add Owner button', regressionTag, async function (this: CustomWorld) {
  this.addOwnerPage = new AddOwnerPage(this.page);
  await this.page.getByRole('link', { name: 'Add Owner' }).click();
});

When('I enter owner details:', regressionTag, async function (this: CustomWorld, dataTable) {
...
When('I submit the Add Owner form', regressionTag, async function (this: CustomWorld) {
  await this.addOwnerPage.addOwnerButton().click();
});

Then('I should see the new owner added', regressionTag, async function (this: CustomWorld) {
...
When('I enter last name {string} in the search field', regressionTag, async function (this: CustomWorld, lastName: string) {
  await this.ownersPage.lastNameInput().fill(lastName);
});

When('I click the Find Owner button', regressionTag, async function (this: CustomWorld) {
  await this.ownersPage.findOwnerButton().click();
});

Then('I should see the owner {string} with address {string}, city {string}, and telephone {string} in the owners table', regressionTag, async function (this: CustomWorld, fullName: string, address: string, city: string, telephone: string) {
...
When('I click the owner link {string} with address {string}', regressionTag, async function (this: CustomWorld, fullName: string, address: string) {
...
When('I click the Edit Owner button', regressionTag, async function (this: CustomWorld) {
  const editLink = this.page.getByRole('link', { name: 'Edit Owner' });
  await editLink.waitFor({ state: 'visible', timeout: 5000 });
  await editLink.click();
});

When('I update owner details:', regressionTag, async function (this: CustomWorld, dataTable) {
  const owner = dataTable.hashes()[0];
...
When('I submit the Update Owner form', regressionTag, async function (this: CustomWorld) {
...
Then('I should see the updated owner {string} with address {string}, city {string}, and telephone {string}', regressionTag, async function (this: CustomWorld, fullName: string, address: string, city: string, telephone: string) {
  await expect(this.page.getByText(fullName)).toBeVisible();
...
When('I click the Add New Pet button', regressionTag, async function (this: CustomWorld) {
  await this.page.getByRole('link', { name: 'Add New Pet' }).click();
});

When('I enter pet details:', regressionTag, async function (this: CustomWorld, dataTable) {
...
When('I submit the Add Pet form', regressionTag, async function (this: CustomWorld) {
  await this.page.getByRole('button', { name: 'Add Pet' }).click();
  await this.page.waitForTimeout(1500); // Wait for page to update
});

Then('I should see the new pet {string} with birth date {string} and type {string} for owner {string}', regressionTag, async function (this: CustomWorld, petName, birthDate, type, ownerName) {
...
When('I click the pet name link {string}', regressionTag, async function (petName) {
  const page = this.page;
  // Find the pet link by name in the owner's details page
  await page.click(`text=${petName}`);
});

When('I click the Add Visit button', regressionTag, async function () {
  const page = this.page;
  // Find and click the Add Visit button (link or button)
  await page.click('a[href*="visits/new"], button:has-text("Add Visit")');
});

When('I enter visit details:', regressionTag, async function (dataTable) {
  const page = this.page;
  ...
  if (data.description) {
    ...
    await page.screenshot({ path: 'description-filled.png', fullPage: false });
  }
});

When('I submit the Add Visit form', regressionTag, async function () {
  const page = this.page;
...
Then('I should see the new visit with date {string} and description {string} for pet {string}', regressionTag, async function (date, description, petName) {
  const page = this.page;
